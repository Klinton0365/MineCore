...
export default function EnquiryPage() {
  ...
  if (submitted) {
    return (
      <div className="pt-16 min-h-screen bg-[var(--color-primary)] flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <Link href="/products" className="inline-block mt-6 bg-[var(--color-accent-blue)] text-white px-6 py-3 rounded-lg hover:bg-blue-600 transition-colors">
            Continue Browsing
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            {items.length === 0 ? (
              <div className="bg-[var(--color-primary-light)] border border-white/10 rounded-xl p-12 text-center">
                ...
                <p className="text-[var(--color-text-muted)] text-lg">Your enquiry cart is empty</p>
                <Link href="/products" className="inline-block mt-4 text-[var(--color-accent-blue)] hover:underline">
              ...
              <div className="space-y-3">
                {items.map(item => (
                  <div key={item.id} className="bg-[var(--color-primary-light)] border border-white/10 rounded-xl p-4 flex items-center justify-between">
                    <div>
                      <Link href={`/products/${item.slug}`} className="text-white font-medium hover:text-[var(--color-accent-blue)]">
                        {item.name}
                      </Link>
          ...
          {/* Enquiry Form */}
          <div>
            <form onSubmit={handleSubmit} className="bg-[var(--color-primary-light)] border border-white/10 rounded-xl p-6 sticky top-24">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm text-[var(--color-text-muted)] mb-1">Country *</label>
                ...
                <div>
                  <label className="block text-sm text-[var(--color-text-muted)] mb-1">Message *</label>
                  <textarea required rows={3} value={formData.message} onChange={e => setFormData(f => ({ ...f, message: e.target.value }))}
                    className="w-full bg-[var(--color-secondary)] border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[var(--color-accent-blue)] resize-none" />
                </div>
              </div>

              <button type="submit" disabled={submitting || items.length === 0}
                className="w-full mt-4 bg-[var(--color-accent-blue)] text-white py-3 rounded-lg font-medium hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">

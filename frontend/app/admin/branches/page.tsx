    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowForm(!showForm); setEditId(null); setForm({ name: '', region_code: '', country: '', description: '', address: '', display_logic: '' }); }}
          className="bg-[var(--color-accent-blue)] text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-600 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="bg-[var(--color-primary-light)] border border-white/10 rounded-xl p-6">
          <div className="grid sm:grid-cols-2 gap-4">
            {(['name', 'region_code', 'country', 'description', 'address', 'display_logic'] as const).map(field => (
              <div key={field}>
                <label className="block text-sm text-[var(--color-text-muted)] mb-1 capitalize">{field.replace('_', ' ')}</label>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {loading ? (
          [1,2,3].map(i => <div key={i} className="h-40 bg-[var(--color-secondary)] rounded-xl animate-pulse" />)
        ) : branches.map(branch => (
          <div key={branch.id} className="bg-[var(--color-primary-light)] border border-white/10 rounded-xl p-5">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-white font-semibold">{branch.name}</h3>
                <span className="text-xs font-mono text-[var(--color-accent-blue)]">{branch.region_code}</span>
              </div>
              <span className={`w-2.5 h-2.5 rounded-full ${branch.is_visible ? 'bg-green-500' : 'bg-red-500'}`} />
            </div>
            <p className="text-sm text-[var(--color-text-muted)] mt-2">{branch.country}</p>
            {branch.display_logic && <p className="text-xs text-[var(--color-text-muted)] mt-1">{branch.display_logic}</p>}
            <div className="flex gap-2 mt-4">
              <button onClick={() => startEdit(branch)} className="text-xs text-[var(--color-accent-blue)] hover:underline">Edit</button>
